...
def main(args):
    ...
    if args.technique == 'baseline DENSENET':
        ...
        if(args.showGradCam=='true'):
            GradCam(device, 'TransferResnet', args.GradCamImagePath)
        ...
        test_labels, pred_cls, pred_proba, _, _ = eval_best_model('densenet121',
                                                                  ...
                                                                  num_classes, loss_func_test, test_loader, device, band='RGB')
        if(args.showGradCam=='true'):
            GradCam(device, 'TransferDensenet', args.GradCamImagePath)

    elif args.technique == 'SimCLR':
        pretrainSimCLR(args.CheXPertSSLPath)
        ...
        if (args.showGradCam == 'true'):
            GradCam(device, 'SimCLR', args.GradCamImagePath)


    elif args.technique == 'MoCo':
        pretrainMoCo(args.CheXPertSSLPath)
        ...
        if (args.showGradCam == 'true'):
            GradCam(device, 'MoCo', args.GradCamImagePath)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--showGradCam", required=False, default='false', type=str, choices=['true', 'false'], metavar='showGradCam', help="showGradCam")
    parser.add_argument("--GradCamImagePath", required=False, type=str, metavar='GradCamImagePath', help="GradCamImagePath")
    parser.add_argument("--CheXPertSSLPath", required=True, type=str, metavar='CheXPertSSLPath', help="CheXPertSSLPath")
